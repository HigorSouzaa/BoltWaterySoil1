...
import arduinoModuleService, {
  ArduinoModule,
} from "../services/arduinoModuleService";
...
export const MaintenanceSchedule: React.FC = () => {
  ...
  const loadSectors = useCallback(async () => {
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Erro ao carregar setores";
  ...
  const loadSchedules = useCallback(async () => {
    ...
    try {
      ...
      if (filterStatus !== "all") {
        filteredSchedules = updatedSchedules.filter(
  ...
  const handleSaveSchedule = async () => {
    try {
      ...
      if (editingSchedule) {
        ...
        await maintenanceScheduleService.updateMaintenanceSchedule(
          editingSchedule._id,
          {
            title: scheduleForm.title.trim(),
            description: scheduleForm.description.trim() || undefined,
            scheduled_date: scheduleForm.scheduled_date,
            priority: scheduleForm.priority as
              | "low"
              | "medium"
              | "high"
              | "critical",
  ...
  const handleCompleteSchedule = async (id: string) => {
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Erro ao concluir manutenção";
      error("Erro", errorMessage);
    }
  };
